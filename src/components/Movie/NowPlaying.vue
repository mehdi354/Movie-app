<template>
    <v-row>
        <v-col cols="12">
            <h4 class="single-list-movie-title">Now Playing Movies</h4>
            <hr>
        </v-col>
        <v-col cols="9">
            <v-row>
                ...
                <v-row v-if="nowPlayingFilteredMovies.length == 0 && selectedMovieGenre.length == 0 && selectedRatings.length == 0">
                    <v-col cols="3" class="px-5" v-for="movie in nowPlayingMovies" :key="movie.index">
                        <div class="single-movie-list">
                            <img v-if="movie.poster_path==null" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTgzgTJNzw-Xsu0aP6ulYClijCBs9ktYIKdpZuI8fk75KaOyd0c&s"  alt="">
                            <img v-else :src="'https://image.tmdb.org/t/p/w500/' + movie.poster_path" alt="">
                            <h4 class="single-list-movie-title">
                                <router-link  :to="{ name: 'details', params: { id: movie.id, movieDetails: movie,theater:'yes'}}"> {{ movie.title }}  </router-link> 
                            </h4>
                ...
                <v-row v-else-if="(nowPlayingFilteredMovies.length == 0) && (selectedMovieGenre.length >0 || selectedRatings.length > 0)">
                    <v-col cols="12">
                ...
                <v-row v-else-if="nowPlayingFilteredMovies.length > 0 && selectedMovieGenre.length >0 || selectedRatings.length > 0">
                    <v-col cols="3" class="px-5" v-for="movie in nowPlayingFilteredMovies" :key="movie.index">
                        <div class="single-movie-list">
                            <img v-if="movie.poster_path==null" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTgzgTJNzw-Xsu0aP6ulYClijCBs9ktYIKdpZuI8fk75KaOyd0c&s"  alt="">
                            <img v-else :src="'https://image.tmdb.org/t/p/w500/' + movie.poster_path" alt="">
                            <h4 class="single-list-movie-title">
...
export default {
    ...
    methods: {
        ...
        filterByRating(){
            this.$store.dispatch('nowplayingfilterPopuler', this.selectedRatings);
        },
    },
    computed: {
        nowPlayingMovies() {
            return this.$store.getters.newComingMovieList;
        },
        allGenre() {
            return this.$store.getters.allGenres;
        },
        nowPlayingFilteredMovies() {
            return this.$store.getters.nowplayingfilteredMovies;
        }
    ...
    created() {
        return this.$store.dispatch('nowPlayingMoviess');
    },
}
</script>