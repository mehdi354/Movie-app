<template>
    <v-row>
        <v-col cols="12">
            <h4 class="single-list-movie-title">Popular Movies</h4>
            <hr>
        </v-col>
        <v-col cols="9">
            <v-row>
                ...
                <v-row v-if="filteredMovies.length == 0 && selectedMovieGenre.length == 0 && selectedRatings.length == 0">
                    <v-col cols="3" class="px-5" v-for="movie in popularMovies" :key="movie.index">
                ...
                <v-row v-else-if="(filteredMovies.length == 0) && (selectedMovieGenre.length >0 || selectedRatings.length > 0)">
                    <v-col cols="12">
                ...
                <v-row v-else-if="filteredMovies.length > 0 && selectedMovieGenre.length >0 || selectedRatings.length > 0">
                    <v-col cols="3" class="px-5" v-for="movie in filteredMovies" :key="movie.index">
                        <div class="single-movie-list">
                            <img v-if="movie.poster_path==null" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTgzgTJNzw-Xsu0aP6ulYClijCBs9ktYIKdpZuI8fk75KaOyd0c&s"  alt="">
                            <img v-else :src="'https://image.tmdb.org/t/p/w500/' + movie.poster_path" alt="">
                            <h4 class="single-list-movie-title">
...
export default {
    ...
    methods: {
        ...
        filterByGenre(){
            this.$store.dispatch('filterPopuler', this.selectedMovieGenre);
        },
        filterByRating(){
            this.$store.dispatch('filterPopuler', this.selectedRatings);
        },
    },
    computed: {
        popularMovies() {
        ...
        // genresname() {
        //     return this.$store.getters.genresnames;
        // },
    ...
    created() {
        return this.$store.dispatch('popularMovies');
    },
}
</script>