    ...
    <v-row>
        ...
        <v-col cols="9">
            <v-row>
                ...
                <v-row v-if="upcomingfilteredMovies.length == 0 && selectedMovieGenre.length == 0 && selectedRatings.length == 0">
                    <v-col cols="3" class="px-5" v-for="movie in upcomingMovieResult" :key="movie.index">
                ...
                <v-row v-else-if="(upcomingfilteredMovies.length == 0) && (selectedMovieGenre.length >0 || selectedRatings.length > 0)">
                    <v-col cols="12">
                ...
                <v-row v-else-if="upcomingfilteredMovies.length > 0 && selectedMovieGenre.length >0 || selectedRatings.length > 0">
                    <v-col cols="3" class="px-5" v-for="movie in upcomingfilteredMovies" :key="movie.index">
                        <div class="single-movie-list">
                            <img :src="'https://image.tmdb.org/t/p/w500/' + movie.poster_path" alt="">
                            <h4 class="single-list-movie-title">
                                <router-link  :to="{ name: 'details', params: { id: movie.id, movieDetails: movie}}"> {{ movie.title }}  </router-link> 
...
export default {
    ...
    methods: {
        ...
        filterByGenre(){
            this.$store.dispatch('upcomingfilter', this.selectedMovieGenre);
        },
        filterByRating(){
            this.$store.dispatch('upcomingfilter', this.selectedRatings);
        },
    },
    computed: {
        upcomingMovieResult() {
            return this.$store.getters.upcomingMovieResult;
        },
        allGenre() {
            return this.$store.getters.allGenres;
        },
        upcomingfilteredMovies() {
            return this.$store.getters.upcomingfilteredMovies;
        }
    ...
    created() {
        return this.$store.dispatch('upCoingMovies');
    },
}
</script>